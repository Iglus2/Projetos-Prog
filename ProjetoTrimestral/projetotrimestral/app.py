...
class AdicionarTarefa(FlaskForm):
    ...
    descricao = PasswordField(
        "Descricao: ",
        validators=[DataRequired(message="É obrigatório colocar uma descricao!")],
    )
...
def atualizar_tarefa():
    ...
    if not pedidos_do_user:
        ...
        print(
            f"Nova Tarefa Atualizada! Tarefa: {Tarefinha} |  Task : {nova_tarefa_escolhida}"
        )
...
def finalizar_tarefa():
    ...
    if indice >= 0 and indice < len(lista_tarefas):
        ...
        print(
            f"Tarefa Finalizada! Tarefa: {tarefa_finalizada['descricao']} |  Task : {tarefa_finalizada['tarefa']}"
        )
    else:
        return f"""
